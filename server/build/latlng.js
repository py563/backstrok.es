...
var LatLng = function LatLng(config) {
      ...
      performRequest(host, url, function (error, result) {
        if (error) {
          ...
          if (!entry.lat || !entry.lng) {
            ...
            callback({
              "name": "RuntimeError",
              "description": "The specified zip code " + zip + " either did not resolve to a valid location, or there was " + "an error in the Yahoo service.",
              "details": result
            });
          } else {
  ...
  function getDistance(start, end) {
    ...
    var radius = 6371,
        ...
        a = Math.sin(dLat / 2) * Math.sin(dLat / 2) + Math.cos(start.lat.toRad()) * Math.cos(end.lat.toRad()) * Math.sin(dLon / 2) * Math.sin(dLon / 2),
